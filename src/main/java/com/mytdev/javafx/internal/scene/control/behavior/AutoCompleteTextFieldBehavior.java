...
import com.mytdev.javafx.internal.scene.control.skin.AutoCompleteTextFieldSkin;
import com.mytdev.javafx.scene.control.AutoCompleteTextField;
import com.mytdev.javafx.scene.control.AutoCompleteTextField.DataToString;
...
public final class AutoCompleteTextFieldBehavior<T> extends BehaviorBase<AutoCompleteTextField<T>> {
        ...
        public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
            ...
            if (newValue != null && oldValue.length() > 0 && newValue.startsWith(oldValue)) {
                ...
                for (T item : autoCompleteList) {
                    String str = dataToString.toString(item,getControl()).toLowerCase();
    ...
    public void initialize(AutoCompleteTextFieldSkin<T> skin) {
        ...
        control.dataToStringProperty().addListener(new ChangeListener<DataToString<T>>() {
            @Override
            public void changed(ObservableValue<? extends DataToString<T>> observable, DataToString<T> oldValue, DataToString<T> newValue) {
                listComparator = new DataToStringComparator<>(newValue);
                FXCollections.sort(autoCompleteList, listComparator);
            }
        });
    ...
    private void selectAutoCompleteItem() {
        ...
        if (item != null) {
            skin.getTextField().setText(getControl().getDataToString().toString(item,getControl()));
            skin.getTextField().requestFocus();
        ...
        InvalidationListener listener = new InvalidationListener() {
            ...
            public void invalidated(Observable ove) {
                if (cell.getItem() != null && cell.isFocused()) {
                    ...
                    updatingField = true;
                    String string = getControl().getDataToString().toString(cell.getItem(),getControl());
